...
public class Day3part2 {
	...
	public static void main(String[] args) {
		...
		Scanner sc = new Scanner(System.in); //answer: 2973
		...
		for (int i=0; i<100; i++) {
			...
			String rucksack2 = sc.next();
			String rucksack3 = sc.next();
			for (int j=0; j<rucksack.length(); j++) {
				for (int k=0; k<rucksack2.length(); k++) {
					for (int l=0; l<rucksack3.length(); l++) {
						if (rucksack.charAt(j)==rucksack2.charAt(k) && rucksack.charAt(j)==rucksack3.charAt(l)&&bool==false) {
						bool = true;
